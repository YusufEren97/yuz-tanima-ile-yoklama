...
public class AuthController {
    ...
    public String girisFormu(@RequestParam(required = false) String hata,
            ...
            Model model) {
        if (hata != null) {
            model.addAttribute("hata", "Email veya şifre hatalı!");
        }
        if (cikis != null) {
    ...
    public String kayitOl(@RequestParam String email,
        ...
        if (kullaniciService.ogrenciNoMevcutMu(ogrenciNo)) {
            redirect.addFlashAttribute("hata", "Bu öğrenci numarası zaten kayıtlı!");
            return "redirect:/kayit";
        }
        ...
        kullaniciService.kaydet(yeniKullanici);
        redirect.addFlashAttribute("mesaj", "Kayıt başarılı! Şimdi giriş yapabilirsiniz.");
        return "redirect:/giris";
    }
